...
var span = 5
 function setup() {
  ...
  radioElement.style("fone-size","30px")
...
function draw() {
  ...
  push()
    translate(width/2-capture_width/2-capture_height/2) //把原點移到(width/2-320,height/2-240)
    ...
    for (var x = 0; x<captureGraphics.width;x=x+span){
      ...
      if(radioElement.value()=="亮度"){
        ...
        ellipse(x,y,span*map(bk,0,255,0,1))  //透過map指令，bk在0~255間，依照比率設定0~1r間的數字
      }
